...
function Secure-Users {
    ...
    if ($unauthorizedUsers.Count -gt 0) {
        ...
        Write-Host "`nREVIEWING EACH USER" -ForegroundColor Magenta
        
        foreach ($user in $unauthorizedUsers) {
            Write-Host "`n" -ForegroundColor Cyan
            Write-Host "User: $user" -ForegroundColor Yellow
            Write-Host "1 = Disable" -ForegroundColor Green
            Write-Host "2 = Delete" -ForegroundColor Red
    ...
    if ($unauthorizedAdmins.Count -gt 0) {
        ...
        Write-Host "`nTAKE SCREENSHOTS FOR EVIDENCE" -ForegroundColor Magenta
        Write-Host "`nREVIEWING EACH ADMIN INDIVIDUALLY" -ForegroundColor Magenta
            ...
            switch ($adminChoice) {
                ...
                "2" {
                    ...
                    try {
                        ...
                        Log-Action "Disabled admin account: $adminUsername"
                        Write-Host "Disabled: $adminUsername" -ForegroundColor Green
                    } catch {
                ...
                "3" {
                    Log-Action "Kept admin: $($admin.Name)"
                    Write-Host "Keeping: $($admin.Name)" -ForegroundColor Cyan
                }
                default {
                    Log-Action "Invalid choice for $($admin.Name) - keeping admin"
                    Write-Host "Invalid choice - keeping admin" -ForegroundColor Yellow
                }
            }
        }
    } else {
    ...
    if ($checkDomain -eq "yes") {
        try {
            ...
            if ($unauthorizedDomainUsers.Count -gt 0) {
                ...
                Write-Host "`nREVIEWING EACH DOMAIN USER INDIVIDUALLY" -ForegroundColor Magenta
            ...
            Write-Host "`nChecking Domain Admins group..." -ForegroundColor Cyan
            ...
            try {
                ...
                if ($unauthorizedDomainAdmins.Count -gt 0) {
                    ...
                    Write-Host "`nREVIEWING EACH DOMAIN ADMIN INDIVIDUALLY" -ForegroundColor Magenta
            ...
            Log-Action "Domain user management failed: $_"
        }
    } else {
